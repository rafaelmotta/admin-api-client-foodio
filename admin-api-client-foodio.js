  ...
  return new ((function (_ApiBase) {
    ...
    _createClass(PageApi, [{
      ...
      value: function remove(page) {
        return Restangular.one('companies', this.company.id).one('pages', page.id).remove();
      }
    }]);
...
var service = function service(Restangular, ApiBase, $q) {
  return new ((function (_ApiBase) {
    _inherits(PrinterApi, _ApiBase);

    function PrinterApi() {
      _classCallCheck(this, PrinterApi);

      _get(Object.getPrototypeOf(PrinterApi.prototype), 'constructor', this).apply(this, arguments);
    }

    _createClass(PrinterApi, [{
      key: 'fetch',
      value: function fetch() {
        return Restangular.one('companies', this.company.id).one('stores', this.store.id).one('printers').get();
      }
    }, {
      key: 'create',
      value: function create(printer) {
        return Restangular.one('companies', this.company.id).one('stores', this.store.id).post('printers', { printer: printer });
      }
    }, {
      key: 'update',
      value: function update(printer) {
        return Restangular.one('companies', this.company.id).one('stores', this.store.id).one('printers', printer.id).patch({ printer: printer });
      }
    }, {
      key: 'destroy',
      value: function destroy(printer) {
        return Restangular.one('companies', this.company.id).one('stores', this.store.id).one('printers', printer.id).remove();
      }
    }]);
